...
#### 만세력 전수 검증 결과 (2026-08-20)

`@fullstackfamily/manseryeok`(외부 만세력 DB)와 1940~2026 **전일 31,777일** 대조 + KASI 공식 분단위 시각/천문 계산으로 불일치 전수 판정:

- **일주: 31,777일 전부 일치. 시주: 5,300건 전부 일치. 연주 단독 불일치 0.**
- 월주 불일치 410건은 **전건 외부 라이브러리 결함**으로 판정 (우리 엔진 오답 0건): 절기 당일을 분 단위가 아닌 날짜 단위로 처리 317건, 절기 하루 조기 적용 82건, 12/31 연주 붕괴 11건.
- astro 폴백 정확도: KASI 공식 시각과 겹치는 2000~2026 절기 324개 중 323개가 **오차 ≤2분(평균 1.4분)** → 2000년 이전 계산도 신뢰 가능.
- 유일한 자체 데이터 오류 발견·수정: `kasi-jeolgi-table.json`의 **2011년 입동이 2012년 값(09:26)의 복제**였음 — KASI 공공 API 원본 자체 오류. 태양 이동 속도 물리 검증(입동→소설 15°/14.65일=1.024°/일 > 물리 최대 1.019°/일)으로 확정, 실제값 03:35로 정정. 수정 전에는 2011-11-08 03:35~09:26 출생자의 월주가 틀렸음.
- 알려진 한계(미수정): 한국 표준시 역사 미반영 — 1954-03-21~1961-08-09는 UTC+8:30(절기 벽시계 30분 차), 서머타임 시행기(1948~60 일부, 1987~88). 해당 시기 절기 경계 ±30~60분 창의 출생자만 영향.
...
- **가족 인연/복덕**(App.jsx:238~251): 해당 십성 쌍 개수 기반 3등급 + 사주 해시 시드 PRNG 지터(같은 사주 = 같은 점수, 2026-08-20 수정). 아버지=재성, 어머니=인성, 배우자=남/여별 재성/관성, 자식=식상.
- **재물/직장/학문 점수** `relScore`(App.jsx:229): 십성 쌍 개수×15 + 신살 길신 보너스 − 충 패널티 + 시드 PRNG 지터(0~7).
...
### ✅ 수정 완료 — 2026-08-20 (적대적 검증 워크플로로 확정 후 수정)

| # | 문제 | 수정 내용 |
|---|---|---|
| 1 | 원진표가 육해 테이블 복사본 (4쌍 불일치) | 통설 6쌍(子未·丑午·寅酉·卯申·辰亥·巳戌)으로 교체 (saju-tables.js) |
| 2 | 귀문표 절반이 통설과 상이 | 통설 6쌍(子酉·丑午·寅未·卯申·辰亥·巳戌)으로 교체 |
| 3 | 대운수 계산 버그 3종 (평균 절기일 근사 / 월초 출생 절기 오인 / 12월생 순행 항상 10세 캡) | saju-core.js에 `findPrevNextJeolgi` 신설(KASI 정밀 시각 + astro 폴백), 대운시작나이가 이를 사용하도록 전면 교체. 회귀: 576케이스 중 300건 정정, 손계산 6케이스 전부 일치 |
| 4 | 진태양시 보정 시각이 연·월주 절기 경계 판정에 유입 (절기 당일 ±32분 구간 연주/월주 오판. 일주 부분은 정자시 통설상 기존 동작이 정답이라 유지) | birthToSaju가 연·월·일주는 KST 원시각, 시주만 보정 시각으로 분리 계산 (App.jsx) |
| 5 | Math.random() 점수 — 재실행마다 점수 상이, 공유 재현 불일치 | 사주 문자열 해시 시드 PRNG(mulberry32)로 교체 — 같은 사주는 항상 같은 점수 |
| 6 | returnMenu 인연 복원 TypeError (`m.profile.key` — 평탄 객체에 없는 필드) | `m.key` + 평탄 필드 참조로 수정, chatTarget에 matchGrade/matchScore/matchReason 포함 |
| 7 | sendMansinChat 오행 요약 "목undefined" (`오행.목` → 실제는 `오행.카운트.목`) | `result.오행.카운트.*` 참조로 수정 |
| 8 | 백호살표 오정의(육충 테이블) + 미사용 — 백호 건강경고 죽은 코드 | `백호간지` 7종(甲辰·乙未·丙戌·丁丑·戊辰·壬戌·癸丑)으로 교체하고 신살판정에 기둥 판정 추가 → 기존 건강경고·신살 상세 해설이 살아남 |
| — | (부수 수정) 세운·12월운이 입춘 경계 무시 | 현재 세운 연도를 `calculateSaju(...).meta.sajuYear`로 산출 — 1/1~입춘 사이 접속자에게 정확한 세운 표시 |
| — | (부수 수정) `CY=2026`·`cy=2026` 하드코딩, 지장간T 申 여기 己→戊(통설 戊·壬·庚), 반안표=겁살표 복사(미사용 표) | 연도 동적화, 테이블 정정 |

수정하지 않은 것(유파 차이로 버그 아님 판정): 양인표 음간 값(녹후일위 소수설과 일치), 자시 처리 방식(정자시/야자시 유파 선택), `Math.round(diff/3)` 반올림 유파. 검증 근거는 적대적 검증 워크플로 결과(트레이스 포함) 참조.

### 심각 — 남은 항목 없음 (아래는 미해결 목록)
...
- 지장간 테이블 3중 중복(App.jsx 지장간T·정기T·지장간비율 vs saju-tables 지장간 vs saju-deep 지장간해설 — 값은 이제 통일, 소스 단일화 필요), 지장간비율 申[0.33,0.10,0.57]이 통설 월률(戊23%·壬23%·庚53%)과 달리 수(壬) 과소평가.
- 서머타임(1948~60, 1987~88) 미보정, 오늘운세가 브라우저 로컬 시간 기준. 12월운은 세운 연도는 정정됐으나 월 경계는 여전히 양력 월 기준.
- 출생 '분' 입력 없음(birthToSaju가 minute=0 고정) — 진태양시 -32분 보정의 정밀도 제약.
- matches 첫 인사 content가 빈 문자열(App.jsx:3926).
